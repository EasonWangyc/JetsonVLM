...
\begin{itemize}
  ...
  \item \textbf{LoRA 领域适配}：按连续采集来源组隔离训练、验证和冻结测试集，基于 80 张独立泊车图像构造弱监督 SFT 数据；在 RTX 4090 D 上对 \textbf{Qwen3-VL-2B-Instruct} 语言 attention 注入 LoRA，仅训练 \textbf{642 万参数（0.301\%）}，峰值显存 \textbf{5.25 GiB}，完成 adapter 合并、ONNX 导出及 Jetson TensorRT engine 部署
  \item \textbf{质量评测}：在相同 20 张冻结测试集上完成 Base、LoRA adapter、合并模型和板端 engine 对照，严格 JSON 有效率保持 \textbf{100\%}；Jetson LoRA 20/20 样本完成，事件 micro-F1 由 Base FP16 的 \textbf{0.359 提升至 0.389}，并通过分类错误分析定位狭窄通道偏置与遮挡事件漏检
