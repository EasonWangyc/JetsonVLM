...
### 7.7 候选 LoRA 的 80 条服务器开发集评测

候选 adapter 在本地 RTX 4060 上完成 64 条 train 与 16 条 validation 的服务器评测，
两处分片均为严格 JSON `100%`，风险准确率分别为 `57.81%` 和 `62.50%`，事件
micro-F1 均为 `0`，不安全建议率分别为 `32.81%` 和 `18.75%`。合计为 80/80 严格 JSON、
风险准确率 `58.75%`、事件 micro-F1 `0`、不安全建议率 `30.00%`。
